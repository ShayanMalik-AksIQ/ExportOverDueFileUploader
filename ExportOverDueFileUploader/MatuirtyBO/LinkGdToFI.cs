    ...
    public static class LinkGdToFI
    {
        ...
        public static string SyncNewGd(DateTime dateTime, List<string> fis )
        {
            try
            {
                ...
                if (lstgds.Count == 0)
                {
                    Console.WriteLine($"No Gds To Sync");

                    return "No Gds";
                }
                var lstfis = CustomRepo.GetFinancialInstrumentForV20Dates(AppSettings.TenantId, fis).ToList();
                if (lstfis.Count == 0)
                {
                    Console.WriteLine($"No Fis To Sync");
            ...
            {
                Console.WriteLine($"Error In Sync New Gds :{ex.Message}");
