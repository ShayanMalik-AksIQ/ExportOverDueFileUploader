...
namespace ExportOverDueFileUploader.ValidateIqBizLogic
{
    ...
    //            var fid = fiGd.Gds.Count == 1 ? null : fiGd.FiId;
    //            var gdfid = fiGd.Gds.Count > 1 ? fiGd.Gds.FirstOrDefault()?.GdFiId : null;
    ...
    //                    var hsCodeComparision = CompareJsonTokens(hsCode1, hsCode2, false); // false as there'll be no need to calc variance for code.
    ...
    public static class Compression
    {
        ...
        private static List<ComparisonResult> ProcessFields(string gdJson, string fiJson, long ReqStatusId, ComparatorSetting setting, string BaseFeild = "FI", long? fId = null, long? gId = null)
        {
            try
            {
                ...
                if (setting.ValidationType == "Delivery Term")
                {
